...
export interface CollectionDiff {
  ...
  inserts: Item[];
  /** Existing records whose content changed — written with an upsert. */
  updates: Item[];
 ...
 * read access to the row it creates (so you can notify someone else), whereas an
 * upsert is also checked against the table's SELECT policy.
 */
export function diffCollection(prev: unknown[], next: unknown[]): CollectionDiff {
  const before = new Map<string, string>();
  prev.filter(isItem).forEach((item) => before.set(item.id, JSON.stringify(item)));
  ...
  next.filter(isItem).forEach((item) => {
    ...
    else if (old !== JSON.stringify(item)) updates.push(item);
  });
