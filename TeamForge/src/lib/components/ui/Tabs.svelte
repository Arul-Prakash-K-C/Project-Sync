...
<div role="tablist" class="flex items-center gap-1 p-1 rounded-xl border border-border bg-muted/40 {className}">
  {#each items as item}
    <button
      role="tab"
      ...
      class="flex-1 py-2 px-4 text-sm font-medium rounded-lg transition-all duration-200 cursor-pointer
        {active === item.value
